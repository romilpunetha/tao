...
impl EntEventBuilder {
    ...
    pub async fn save(self, tao: &Tao) -> AppResult<EntEvent> {
        ...
        let generated_id = tao.obj_add("ent_event".to_string(), data, None).await?;

        // Create final entity with generated ID
        let mut final_entity = entity;
        final_entity.id = generated_id;

        println!("✅ Created EntEvent with TAO ID: {}", generated_id);
