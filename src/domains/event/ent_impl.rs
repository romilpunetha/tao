...
impl EntEvent {
    ...
    pub(crate) async fn from_tao_object(tao_obj: TaoObject) -> AppResult<Option<EntEvent>> {
        ...
        let entity = EntEvent::read_from_in_protocol(&mut protocol)
    ...
    pub async fn get_attendees(&self) -> AppResult<Vec<EntUser>> {
        ...
        let neighbor_ids = tao
            .get_neighbor_ids(self.id(), "attendees".to_string(), Some(100))
            .await?;
    ...
    pub async fn get_related_posts(&self) -> AppResult<Vec<EntPost>> {
        let tao = get_global_tao()?.clone();
        let neighbor_ids = tao
            .get_neighbor_ids(self.id(), "related_posts".to_string(), Some(100))
            .await?;
    ...
    pub async fn count_related_posts(&self) -> AppResult<i64> {
        let tao = get_global_tao()?.clone();
        let count = tao
            .assoc_count(self.id(), "related_posts".to_string())
            .await?;
        Ok(count as i64)
    }
}
