...
pub type ENTGROUP_NAME = String;

pub type ENTGROUP_DESCRIPTION = String;
...
impl EntGroup {
  pub fn new<F3>(id: i64, name: String, description: F3, created_time: i64) -> EntGroup where F3: Into<Option<String>> {
    EntGroup {
      id,
      name,
...
impl TSerializable for EntGroup {
  fn read_from_in_protocol(i_prot: &mut dyn TInputProtocol) -> thrift::Result<EntGroup> {
    ...
    i_prot.read_struct_end()?;
    verify_required_field_exists("EntGroup.id", &f_1)?;
    verify_required_field_exists("EntGroup.name", &f_2)?;
    verify_required_field_exists("EntGroup.created_time", &f_4)?;
    let ret = EntGroup {
